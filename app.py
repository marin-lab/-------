...
def convert_yen(value):
    try:
        if '万円' in value:
            return float(value.replace('万円', '').replace(' ', '')) * 10000
        return value
    except ValueError:
        return value
        ...
        for child in mother:
            ...
            for room in rooms.find_all(class_='js-cassette_link'):
                ...
                for id_, grandchild in enumerate(room.find_all('td')):
                    if id_ == 2:
                        rent_text = grandchild.find(class_='cassetteitem_other-emphasis ui-text--bold').text.strip()
                        data_room.append(convert_yen(rent_text))
                    elif id_ == 3:
                        emphasis_element = grandchild.find(class_='cassetteitem_other-emphasis ui-text--bold')
                        if emphasis_element:
                            data_room.append(emphasis_element.text)
                        admin_element = grandchild.find(class_='cassetteitem_price cassetteitem_price--administration')
                        if admin_element:
                            data_room.append(admin_element.text)
                    elif id_ == 4:
                        deposit_element = grandchild.find(class_='cassetteitem_price cassetteitem_price--deposit')
                        if deposit_element:
                            data_room.append(convert_yen(deposit_element.text))
                        gratuity_element = grandchild.find(class_='cassetteitem_price cassetteitem_price--gratuity')
                        if gratuity_element:
                            data_room.append(convert_yen(gratuity_element.text))
                    elif id_ == 5:
                        madori_element = grandchild.find(class_='cassetteitem_madori')
                        if madori_element:
                            data_room.append(madori_element.text)
                        menseki_element = grandchild.find(class_='cassetteitem_menseki')
                        if menseki_element:
                            data_room.append(menseki_element.text)
                    elif id_ == 8:
